...
class DouyinProcessor:
    ...
    def parse_share_url(self, share_text: str, ratio: str = "default") -> dict:
        ...
        if desc:
            ...
            desc = re.sub(r'_+', '_', desc).strip('_')[:50]
    ...
    def download_video(self, video_info: dict, output_dir: Optional[Path] = None, show_progress: bool = True) -> Path:
        ...
        if video_info['title']:
            filename = f"{video_info['video_id']}_{video_info['title']}.mp4"
        else:
        ...
        with open(filepath, 'wb') as f:
            for chunk in response.iter_content(chunk_size=8192):
                if chunk:
                    ...
                    now = time.monotonic()
                    if show_progress and total_size > 0 and now - last_print_time >= 1:
                        last_print_time = now
...
def extract_text(share_link: str, api_key: Optional[str] = None, output_dir: Optional[str] = None,
    ...
    if output_dir:
        ...
        if save_video:
            if video_info['title']:
                saved_video_path = video_folder / f"{video_info['video_id']}_{video_info['title']}.mp4"
            else:
